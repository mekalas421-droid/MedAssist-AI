  ...
  return (
    <DashboardShell navItems={navWithLogout} role="clinic">
      <div className="space-y-6">
        <PageHeader
          title="Clinical Decision Support — Provider Review"
          subtitle="Review, approve, or adjust AI disease predictions & recommendations for clinical accuracy."
          breadcrumbs={[{ label: "Dashboard", href: "/provider/dashboard" }, { label: "Reports & Review" }]}
      ...
      {selectedReport && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-2xl rounded-2xl p-6 text-slate-200 max-h-[90vh] overflow-y-auto space-y-5 animate-fadeUp shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div>
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  <ShieldCheck className="text-indigo-400" /> Clinical Review — Report #{selectedReport.id.slice(0, 8)}
                </h3>
            ...
            {/* AI Predictions */}
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">AI Disease Predictions</h4>
            ...
            <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-800 flex items-center justify-between">
              <div>
                <span className="text-xs text-slate-400 uppercase font-bold block">Assessed Risk Category</span>
              ...
              <div>
                <span className="text-xs text-slate-400 uppercase font-bold block">Composite Score</span>
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">
                Physician Clinical Notes & Modifications
              </label>
              <textarea
                rows={4}
                value={doctorNotes}
                onChange={e => setDoctorNotes(e.target.value)}
                placeholder="Enter clinician diagnostic notes, treatment adjustments, or patient guidance..."
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
              ...
              >
                <CheckCircle size={15} /> Approve Clinical Report
              </button>
            </div>
          </div>
        </div>
